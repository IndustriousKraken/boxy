...
pub const BoxyTheme = struct {
    ...
    tl: ?[]const u8 = null,  // top-left corner (e.g., ╔ ┌ ╭)
    tr: ?[]const u8 = null,  // top-right corner (e.g., ╗ ┐ ╮)
    bl: ?[]const u8 = null,  // bottom-left corner (e.g., ╚ └ ╰)
    br: ?[]const u8 = null,  // bottom-right corner (e.g., ╝ ┘ ╯)
...
pub const InnerBorders = struct {
    ...
    cross: []const u8 = "┼",       // Where h and v meet (internal cross junction)
    
    // T-junctions for partial inner borders (NOT YET IMPLEMENTED)
    // These would be used for partial dividers that don't span the full width/height
    // For example: merged cells, subtables, or partial row/column dividers
    t_down: []const u8 = "┬",      // ┬ shape - partial horizontal divider meets top of cell
    t_up: []const u8 = "┴",        // ┴ shape - partial horizontal divider meets bottom of cell  
    t_right: []const u8 = "├",     // ├ shape - partial vertical divider meets left side of cell
    t_left: []const u8 = "┤",      // ┤ shape - partial vertical divider meets right side of cell
...
/// These are the T-shaped characters used when columns/rows meet the box edges
pub const Junctions = struct {
    top: []const u8 = "┬",         // Where columns meet top border (e.g., ╤ ┬ ╦)
    bottom: []const u8 = "┴",      // Where columns meet bottom border (e.g., ╧ ┴ ╩)
    left: []const u8 = "├",        // Where rows meet left border (e.g., ╟ ├ ╠)
    right: []const u8 = "┤",       // Where rows meet right border (e.g., ╢ ┤ ╣)
};
